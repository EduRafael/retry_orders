...
const get_operator = async (email, orderId) => {
  ...
  const { error, type } = await get_sales_order(userId, orderId);
...
const get_vtex = async (email, orderId) => {
  ...
  if (!operator_vtex || operator_vtex.length < 1) {
    ...
    return { operator: null, error: true, type: MESSAGE.ERROR.OPERATOR_VTEX_DNF };
  }
  ...
  if (!parsed.firstName || !parsed.lastName || !parsed.document) {
    ...
    return { operator: parsed, error: true, type: MESSAGE.ERROR.OPERATOR_VTEX_FN };
  }

  return { operator: parsed, error: false, type: '' };
};

const get_sales_order = async (userId, orderId) => {
  const params = {
